...
import Suppliers from './pages/Suppliers'; // Halaman Baru
...
const NAV_ITEMS = [
  ...
  { path: '/suppliers', label: 'Supplier', icon: 'fa-truck-field', roles: ['owner'] }, // Hanya Owner
...
const SidebarItem = ({ path, label, icon, isCollapsed }: any) => (
  ...
  >
    <div className="w-6 text-center"><i className={`fa-solid ${icon}`}></i></div>
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-800">
           <div className={`flex items-center gap-2 overflow-hidden ${!isSidebarOpen && !isMobile ? 'hidden' : ''}`}>
             <div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center font-bold text-lg text-white overflow-hidden shrink-0">
               {appSettings.logoUrl && appSettings.showLogo ? (
                 <img src={appSettings.logoUrl} alt="Logo" className="w-full h-full object-cover" />
               ) : (
                 appSettings.storeName ? appSettings.storeName.charAt(0).toUpperCase() : 'W'
               )}
             </div>
             <span className="font-bold text-lg tracking-tight truncate">{appSettings.storeName || 'Warung POS'}</span>
           </div>
           {!isMobile && <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="text-slate-400 hover:text-white p-1"><i className={`fa-solid ${isSidebarOpen ? 'fa-chevron-left' : 'fa-bars'}`}></i></button>}
        </div>
        <nav className="flex-1 p-3 overflow-y-auto">
          {allowedNavItems.map(item => <SidebarItem key={item.path} {...item} isCollapsed={!isSidebarOpen && !isMobile} />)}
        </nav>
        <div className="p-4 border-t border-slate-800 space-y-2">
          <div className={`flex items-center gap-3 ${!isSidebarOpen && !isMobile ? 'justify-center' : ''}`}>
             <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 to-emerald-500 flex items-center justify-center text-[10px] font-bold">
                {profile.role === 'owner' ? 'OW' : 'KS'}
             </div>
             {(isSidebarOpen || isMobile) && <div className="flex-1 overflow-hidden"><p className="text-sm font-medium truncate">{profile.displayName}</p><p className="text-[10px] text-slate-500 truncate">ID: {profile.warungId}</p></div>}
          </div>
          {(isSidebarOpen || isMobile) && <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 text-xs text-red-400 hover:bg-red-500/10 hover:text-red-300 rounded-lg transition-colors"><i className="fa-solid fa-right-from-bracket"></i><span>Logout</span></button>}
        </div>
      </aside>
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        <header className="lg:hidden h-16 bg-white border-b border-gray-200 flex items-center px-4 justify-between shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="p-2 -ml-2 text-gray-600"><i className="fa-solid fa-bars text-xl"></i></button>
        ...
        <div className="flex-1 overflow-auto p-4 lg:p-6 relative">
          <Routes>
            ...
            <Route path="/pos" element={<PinGuard><POS /></PinGuard>} />
            <Route path="/products" element={<Products role={profile.role} />} />
            <Route path="/inventory" element={<Inventory />} />
            <Route path="/customers" element={<Customers />} />
            <Route path="/suppliers" element={<Suppliers />} /> {/* Route Baru */}
            {profile.role === 'owner' && <><Route path="/reports" element={<PinGuard><Reports /></PinGuard>} /><Route path="/settings" element={<PinGuard><Settings /></PinGuard>} /></>}
