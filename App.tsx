...
const Layout: React.FC<{ user: User }> = ({ user }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-100 overflow-hidden">
      ...
      <aside 
        className={`fixed lg:static inset-y-0 left-0 z-30 bg-slate-900 text-white transition-all duration-300 ease-in-out flex flex-col ${
          isSidebarOpen ? 'w-64 translate-x-0' : isMobile ? '-translate-x-full w-64' : 'w-20'
        }`}
      >
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-800">
           <div className={`flex items-center gap-2 overflow-hidden ${!isSidebarOpen && !isMobile ? 'hidden' : ''}`}>
             {appSettings.logoUrl ? (
               <img src={appSettings.logoUrl} alt="Logo" className="w-8 h-8 rounded-lg object-contain bg-white" />
             ) : (
             ...
             <span className="font-bold text-lg tracking-tight truncate">
               {appSettings.storeName || 'Warung POS'}
             </span>
           </div>
           {!isMobile && (
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="flex-1 overflow-auto p-4 lg:p-6 relative">
          <Routes>
            ...
            <Route path="/reports" element={<PinGuard><Reports /></PinGuard>} />
            <Route path="/settings" element={<PinGuard><Settings /></PinGuard>} />
