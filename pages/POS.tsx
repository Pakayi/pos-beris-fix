      ...
      <Card className="w-full lg:w-96 flex flex-col border-l border-gray-200 shadow-lg h-1/2 lg:h-full">
        <div className="p-4 border-b border-gray-100 bg-gray-50 rounded-t-xl space-y-3">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => setShowCustomerModal(true)} className="text-blue-600 text-[10px] flex items-center gap-1 bg-blue-50 border border-blue-100 px-2 py-1 rounded-lg font-bold">
              <i className="fa-solid fa-user"></i> {selectedCustomer ? selectedCustomer.name : "Pilih Pelanggan"}
            </button>
          </div>
        </div>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200 rounded-b-xl">
          ...
          <Button className="w-full py-3 text-lg shadow-lg shadow-blue-500/20" disabled={cart.length === 0} onClick={() => setShowCheckout(true)}>
            Bayar
          </Button>
        </div>
      </Card>
      ...
      <Modal isOpen={showCheckout} onClose={() => setShowCheckout(false)} title="Metode Pembayaran">
        <div className="space-y-4">
          <div className="flex bg-slate-100 p-1 rounded-xl mb-4">
            <button onClick={() => setPaymentMethod("cash")} className={`flex-1 py-2 text-[10px] font-black rounded-lg transition-all ${paymentMethod === "cash" ? "bg-white text-blue-600 shadow-sm" : "text-slate-500"}`}>
              TUNAI
            </button>
            <button onClick={() => setPaymentMethod("qris")} className={`flex-1 py-2 text-[10px] font-black rounded-lg transition-all ${paymentMethod === "qris" ? "bg-white text-blue-600 shadow-sm" : "text-slate-500"}`}>
              QRIS
            </button>
            <button onClick={() => setPaymentMethod("debt")} className={`flex-1 py-2 text-[10px] font-black rounded-lg transition-all ${paymentMethod === "debt" ? "bg-red-600 text-white shadow-sm" : "text-slate-500"}`}>
              HUTANG
            </button>
          </div>
            ...
            <div className="p-4 bg-red-50 border border-red-100 rounded-xl space-y-2">
              ...
              {selectedCustomer ? (
                ...
                <button
                  ...
                  className="w-full py-2 bg-white text-red-600 text-xs font-bold border border-red-200 rounded-lg"
                >
          ...
          <Button onClick={handleCheckout} className="w-full py-3 mt-2" disabled={(paymentMethod === "cash" && parseInt(amountPaid) < calculations.total) || (paymentMethod === "debt" && !selectedCustomer)}>
          ...
          {filteredCustomers.map((c) => (
            <button
              ...
              onClick={() => {
                ...
                if (paymentMethod === "debt") setShowCheckout(true);
              }}
        ...
        <div className="text-center space-y-4">
          ...
          <div>
            <p className="text-slate-500 text-sm">
              Pembayaran via <span className="font-bold text-slate-800 uppercase">{lastTransaction?.paymentMethod}</span>
            </p>
            <p className="text-3xl font-black text-slate-900">Rp {lastTransaction?.totalAmount.toLocaleString("id-ID")}</p>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <Button onClick={handlePrint} icon="fa-print" className="w-full">
              Cetak Struk
            </Button>
            <Button onClick={() => setShowSuccessModal(false)} variant="secondary" className="w-full">
              Tutup
            </Button>
          </div>
        </div>
...
const generateReceiptHTML = (tx: Transaction, settings: AppSettings, size: "58mm" | "80mm") => {
  ...
  return `<html><head><style>@page{margin:0;size:${width} auto;}body{font-family:'Courier New',monospace;width:${width};margin:0;padding:5px;font-size:10px;}.center{text-align:center;}.bold{font-weight:bold;}.line{border-top:1px dashed #000;margin:5px 0;}.row{display:flex;justify-content:space-between;}</style></head><body><div class="center bold">${
    settings.storeName
  }</div><div class="line"></div><div class="row"><span>No: ${
    tx.id
