    ...
    if (showScanner) {
      ...
      return () => {
        ...
        if (scannerInstanceRef.current?.isScanning) {
          scannerInstanceRef.current.stop().then(() => scannerInstanceRef.current?.clear()).catch(console.error);
        }
      };
    }
  }, [showScanner]);
    ...
    <div className="h-[calc(100vh-6rem)] flex flex-col lg:flex-row gap-4">
      ...
      <Card className="w-full lg:w-96 flex flex-col border-l border-gray-200 shadow-lg h-1/2 lg:h-full">
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200 rounded-b-xl">
          ...
          <Button className="w-full py-3 text-lg" disabled={cart.length === 0} onClick={() => setShowCheckout(true)}>Bayar</Button>
        </div>
      </Card>
      ...
      <Modal isOpen={showCheckout} onClose={() => setShowCheckout(false)} title="Pembayaran">
        <div className="space-y-4">
          <div className="flex bg-slate-100 p-1 rounded-xl mb-4">
             <button onClick={() => setPaymentMethod('cash')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${paymentMethod === 'cash' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500'}`}>TUNAI</button>
             <button onClick={() => setPaymentMethod('qris')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${paymentMethod === 'qris' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500'}`}>QRIS</button>
          </div>
...
const generateReceiptHTML = (tx: Transaction, settings: AppSettings, size: '58mm' | '80mm') => {
  ...
  `).join('');
  return `<html><head><style>@page{margin:0;size:${width} auto;}body{font-family:'Courier New',monospace;width:${width};margin:0;padding:5px;font-size:10px;}.center{text-align:center;}.bold{font-weight:bold;}.line{border-top:1px dashed #000;margin:5px 0;}.row{display:flex;justify-content:space-between;}</style></head><body><div class="center bold">${settings.storeName}</div><div class="center">${settings.storeAddress}</div><div class="line"></div><div class="row"><span>${new Date(tx.timestamp).toLocaleString('id-ID')}</span></div><div class="row"><span>No: ${tx.id}</span></div><div class="line"></div>${itemsHtml}<div class="line"></div><div class="row"><span>Metode</span><span>${tx.paymentMethod.toUpperCase()}</span></div><div class="row bold"><span>TOTAL</span><span>Rp ${tx.totalAmount.toLocaleString('id-ID')}</span></div>${tx.paymentMethod === 'cash' ? `<div class="row"><span>Tunai</span><span>Rp ${tx.cashPaid.toLocaleString('id-ID')}</span></div><div class="row"><span>Kembali</span><span>Rp ${tx.change.toLocaleString('id-ID')}</span></div>` : ''}<div class="line"></div><div class="center">${settings.footerMessage}</div></body></html>`;
};
