...
const POS: React.FC = () => {
  ...
  // Unit Selection State (New feature)
  ...
  useEffect(() => {
    ...
    if (showScanner) {
      ...
      timer = setTimeout(() => {
        // Clear previous instance if any (handling React strict mode double-mount)
        const element = document.getElementById("reader");
        if (element) element.innerHTML = ""; 

        try {
          // REMOVED HTTPS CHECK: Biarkan browser menangani izin secara native.
          ...
          setCameraError("Gagal membuka kamera. Pastikan izin kamera diberikan di pengaturan browser/HP.");
        }
      }, 100);
    }
  ...
  const onScanSuccess = (decodedText: string, decodedResult: any) => {
    ...
    // Cooldown check (1.5 seconds) to prevent double scanning
    ...
    if (product) {
      ...
      // LOGIC UPDATE: Check Unit Count
      if (product.units.length > 1) {
          // If multiple units, PAUSE scanning and ask user
          if (scannerRef.current) {
              scannerRef.current.pause(true); 
          }
          setScanSelection(product);
      } else {
          // If single unit, add directly
      ...
      console.log(`SKU ${decodedText} not found`);
    }
  };
  ...
  const calculations = useMemo(() => {
    ...
    // Tax Calculation (on amount AFTER discount)
  ...
  const handleCheckout = () => {
    ...
    setProducts(db.getProducts()); // Refresh stock
    setAllCustomers(db.getCustomers()); // Refresh customer stats (spent increased)
  ...
  const handleDownloadPDF = () => {
    ...
    const doc = new jsPDF({
      ...
      unit: 'mm',
      format: [paperSize === '58mm' ? 58 : 80, 200] // Continuous roll simulation
    });
  ...
  return (
    <div className="h-[calc(100vh-6rem)] flex flex-col lg:flex-row gap-4">
      ...
      <Card className="w-full lg:w-96 flex flex-col border-l border-gray-200 rounded-none lg:rounded-xl shadow-lg h-1/2 lg:h-full">
        <div className="p-4 border-b border-gray-100 bg-gray-50 rounded-t-xl space-y-3">
          <div className="flex justify-between items-center">
             <h2 className="font-bold text-gray-800"><i className="fa-solid fa-cart-shopping mr-2"></i>Keranjang</h2>
             <span className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full font-bold">{cart.length} item</span>
          </div>
          
          {/* Customer Selection Button */}
          <button 
            onClick={() => { setCustomerSearch(''); setShowCustomerModal(true); }}
          ...
          >
             <div className="flex items-center gap-2 overflow-hidden">
                 ...
                 <span className="truncate font-medium">
                     {selectedCustomer ? `${selectedCustomer.name} (${selectedCustomer.tier})` : 'Pilih Pelanggan / Member'}
                 </span>
             </div>
             {selectedCustomer && (
                 <span 
              ...
              <div key={idx} className="flex justify-between items-start group">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center border rounded-lg bg-gray-50">
                    <button onClick={() => updateQuantity(idx, -1)} className="px-2 py-1 hover:bg-gray-200 rounded-l-lg text-gray-600">-</button>
                    <span className="w-8 text-center text-sm font-medium">{item.quantity}</span>
                    <button onClick={() => updateQuantity(idx, 1)} className="px-2 py-1 hover:bg-gray-200 rounded-r-lg text-gray-600">+</button>
                  </div>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200 rounded-b-xl space-y-2">
          ...
          <div className="space-y-1 text-sm text-gray-600 pb-2 border-b border-gray-200 mb-2">
              ...
              {selectedCustomer && (
                  <div className="flex justify-between text-green-600 font-medium">
                      <span>Diskon {selectedCustomer.tier} ({(calculations.discountRate * 100)}%)</span>
          ...
          <Button 
            className="w-full py-3 text-lg shadow-blue-200 shadow-lg" 
      ...
      <Modal 
        ...
        footer={
          <>
            <Button variant="secondary" onClick={() => setShowCheckout(false)}>Batal</Button>
          ...
          <div className="text-center py-4 bg-blue-50 rounded-lg border border-blue-100 relative overflow-hidden">
            ...
            {calculations.discountAmount > 0 && (
                <p className="text-xs text-green-600 mt-1">
                    Hemat: Rp {calculations.discountAmount.toLocaleString('id-ID')}
                </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[10000, 20000, 50000, 100000].map(val => (
               <button 
                ...
                className="px-2 py-1 text-xs bg-gray-100 hover:bg-gray-200 rounded border border-gray-300 text-gray-600"
               >
                 {val.toLocaleString('id-ID')}
               </button>
            ))}
             <button 
                onClick={() => setAmountPaid(calculations.total.toString())}
                className="col-span-2 px-2 py-1 text-xs bg-blue-100 hover:bg-blue-200 rounded border border-blue-300 text-blue-700"
               >
                 Uang Pas
               </button>
          </div>
      ...
      <Modal
        ...
        footer={<Button variant="secondary" onClick={() => setShowCustomerModal(false)}>Tutup</Button>}
      >
        <div className="space-y-3">
             ...
             <div className="max-h-[300px] overflow-y-auto border rounded-lg divide-y">
                 ...
                 >
                     <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center"><i className="fa-solid fa-user"></i></div>
                     <div>
                 ...
                 {filteredCustomers.map(c => {
                     // Get discounts from settings or default
                     const discounts = settings.tierDiscounts || { bronze: 0, silver: 2, gold: 5 };
                     ...
                     return (
                         ...
                         >
                             <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs border ${
                                 ...
                                 c.tier === 'Silver' ? 'bg-gray-100 text-gray-700 border-gray-300' :
                 ...
                 {filteredCustomers.length === 0 && (
                     <div className="p-4 text-center text-gray-500 text-sm">Pelanggan tidak ditemukan</div>
                 )}
             </div>
        </div>
      </Modal>
        ...
        <div className="text-center space-y-6">
          ...
          <p className="text-xs text-gray-400">
             {printerService.isConnected() 
               ...
               : "ℹ️ Menggunakan dialog cetak standar (Printer Bluetooth tidak terdeteksi)"}
          </p>
        </div>
      </Modal>
      ...
      <Modal
        ...
        title="Scan Barcode"
        footer={<Button variant="secondary" onClick={() => setShowScanner(false)}>Tutup</Button>}
      >
        <div className="flex flex-col items-center justify-center relative">
          {!scanSelection && !cameraError && (
              <>
                <div id="reader" className="w-full max-w-[300px] bg-black rounded-lg overflow-hidden min-h-[250px]"></div>
          ...
          {cameraError && (
            <div className="text-center p-6 bg-red-50 rounded-xl border border-red-100">
                ...
                <div className="text-xs text-left bg-white p-3 rounded border border-red-100 text-gray-600">
                    <strong>Cara mengatasi:</strong>
                    <ul className="list-disc pl-4 mt-1 space-y-1">
                        <li>Pastikan izin Kamera sudah diberikan pada Browser.</li>
                        <li>Pastikan tidak ada aplikasi lain yang sedang menggunakan kamera.</li>
          ...
          {scanSelection && (
             // UNIT SELECTION UI inside Scanner Modal
             <div className="w-full max-w-[300px] py-4">
                 <div className="text-center mb-4">
                     ...
                     <p className="text-xs text-blue-600 mt-1">Pilih satuan yang discan:</p>
                 </div>
                 ...
                 <div className="grid grid-cols-1 gap-2">
                     {scanSelection.units.map((unit, idx) => (
                         <button
                            key={idx}
                            onClick={() => handleUnitSelectFromScan(unit)}
                            className="flex justify-between items-center p-3 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-lg transition-colors"
                         >
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-3 hover:shadow-md transition-shadow flex flex-col justify-between h-full">
      <div>
        ...
        <p className="text-xs text-gray-500 mb-3">Stok: {Math.floor(product.stock)} {product.baseUnit}</p>
      </div>
        ...
        {product.units.map((unit, idx) => (
          ...
          >
            ...
            <span className="font-bold text-gray-700 group-hover:text-blue-700">
              {unit.price.toLocaleString('id-ID', { notation: 'compact', compactDisplay: 'short' })}
            </span>
          </button>
        ))}
      </div>
...
// Helper: Generate Raw HTML for printing (Fallback)
    ...
    <html>
      <head>
        ...
        <style>
          ...
          body { font-family: 'Courier New', monospace; width: ${width}; margin: 0; padding: 5px; font-size: ${fontSize}; color: #000; }
          .center { text-align: center; }
          .right { text-align: right; }
          .bold { font-weight: bold; }
          .line { border-top: 1px dashed #000; margin: 5px 0; }
          .row { display: flex; justify-content: space-between; }
          .logo { max-width: 60%; max-height: 80px; margin: 0 auto 5px auto; display: block; }
        </style>
      </head>
      <body>
        ${settings.showLogo && settings.logoUrl ? `<img src="${settings.logoUrl}" class="logo" />` : ''}
        <div class="center bold" style="font-size: 1.2em; margin-bottom: 2px;">${settings.storeName}</div>
        <div class="center" style="font-size: 0.9em;">${settings.storeAddress}</div>
        <div class="center" style="font-size: 0.9em;">${settings.storePhone}</div>
        <div class="line"></div>
        ...
        <div class="row">
          <span>Diskon</span>
          <span>- Rp ${tx.discountAmount.toLocaleString('id-ID')}</span>
        </div>
        ` : ''}
        <div class="row bold" style="font-size: 1.1em; margin-bottom: 2px;">
          <span>TOTAL</span>
        ...
        <div class="line"></div>
        <div class="center" style="margin-top: 10px;">${settings.footerMessage}</div>
        <div class="center" style="margin-top: 5px; font-size: 0.8em;">-- Terima Kasih --</div>
      </body>
    </html>
  `;
};
