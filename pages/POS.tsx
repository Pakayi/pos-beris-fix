          ...
          <div className="text-center py-4 bg-blue-50 rounded-lg border border-blue-100 relative overflow-hidden">
            ...
            {calculations.discountAmount > 0 && (
                <p className="text-xs text-green-600 mt-1">
                    Hemat: Rp {calculations.total.toLocaleString('id-ID')}
                </p>
            )}
          </div>
          <div>
...
const generateReceiptHTML = (tx: Transaction, settings: AppSettings, size: '58mm' | '80mm') => {
  ...
  `).join('');
  return `<html><head><style>@page { margin: 0; size: ${width} auto; }body { font-family: 'Courier New', monospace; width: ${width}; margin: 0; padding: 5px; font-size: ${fontSize}; color: #000; }.center { text-align: center; }.bold { font-weight: bold; }.line { border-top: 1px dashed #000; margin: 5px 0; }.row { display: flex; justify-content: space-between; }</style></head><body><div class="center bold">${settings.storeName}</div><div class="center">${settings.storeAddress}</div><div class="center">${settings.storePhone}</div><div class="line"></div><div class="row"><span>${new Date(tx.timestamp).toLocaleString('id-ID')}</span></div><div class="row"><span>No: ${tx.id}</span></div><div class="line"></div>${itemsHtml}<div class="line"></div><div class="row bold"><span>TOTAL</span><span>Rp ${tx.totalAmount.toLocaleString('id-ID')}</span></div><div class="row"><span>Tunai</span><span>Rp ${tx.cashPaid.toLocaleString('id-ID')}</span></div><div class="row"><span>Kembali</span><span>Rp ${tx.change.toLocaleString('id-ID')}</span></div><div class="line"></div><div class="center">${settings.footerMessage}</div></body></html>`;
};
