...
const POS: React.FC = () => {
  ...
  const handleCheckout = () => {
    ...
    if (paid < calculations.total) { alert("Pembayaran kurang!"); return; }
    ...
    <div className="h-[calc(100vh-6rem)] flex flex-col lg:flex-row gap-4">
      <div className="flex-1 flex flex-col gap-4 overflow-hidden">
        <div className="flex gap-2">
          ...
          <Button onClick={() => setShowScanner(true)} variant="secondary" icon="fa-barcode">Scan</Button>
        </div>
            ...
            {filteredProducts.map(p => (
              <div key={p.id} className="bg-white border rounded-lg p-3 hover:shadow-md transition-shadow flex flex-col justify-between h-full">
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 mt-1">Stok: {p.stock} {p.baseUnit}</p>
                </div>
                <div className="mt-3 space-y-1">
                  {p.units.map((u, idx) => (
                    <button key={idx} onClick={() => addToCart(p, u)} className="w-full flex justify-between p-1.5 text-[10px] bg-gray-50 border rounded hover:bg-blue-50">
                      <span>{u.name}</span><b>Rp {u.price.toLocaleString('id-ID')}</b>
                    </button>
                  ))}
                </div>
              </div>
      ...
      <Card className="w-full lg:w-96 flex flex-col shadow-xl h-1/2 lg:h-full">
          ...
          {cart.map((item, idx) => (
            <div key={idx} className="flex justify-between items-center bg-white border-b pb-2">
              <div className="flex-1 pr-2">
                ...
                <p className="text-[10px] text-gray-400">{item.quantity} {item.unitName}</p>
              </div>
        ...
        <div className="p-4 bg-gray-100 rounded-b-xl">
          <div className="flex justify-between text-sm mb-1"><span>Subtotal</span><span>Rp {calculations.subtotal.toLocaleString('id-ID')}</span></div>
          {calculations.discountAmount > 0 && <div className="flex justify-between text-xs text-green-600 mb-1"><span>Diskon Member</span><span>- Rp {calculations.discountAmount.toLocaleString('id-ID')}</span></div>}
          <div className="flex justify-between font-bold text-lg text-blue-700 mt-2 border-t pt-2"><span>Total</span><span>Rp {calculations.total.toLocaleString('id-ID')}</span></div>
          <Button className="w-full mt-4 py-3 text-lg" disabled={cart.length === 0} onClick={() => setShowCheckout(true)}>Bayar</Button>
        </div>
      </Card>
      ...
      <Modal isOpen={showCheckout} onClose={() => setShowCheckout(false)} title="Kasir: Bayar">
          <div className="space-y-4">
              ...
              <div className="flex gap-2">
                  <button onClick={() => setAmountPaid(calculations.total.toString())} className="flex-1 py-2 bg-slate-100 rounded-lg text-xs font-bold">Uang Pas</button>
                  <button onClick={() => setAmountPaid("50000")} className="flex-1 py-2 bg-slate-100 rounded-lg text-xs font-bold">50.000</button>
                  <button onClick={() => setAmountPaid("100000")} className="flex-1 py-2 bg-slate-100 rounded-lg text-xs font-bold">100.000</button>
              </div>
              <Button className="w-full py-4 font-bold text-lg" onClick={handleCheckout} disabled={!amountPaid}>PROSES BAYAR</Button>
          </div>
      </Modal>

      <Modal isOpen={showSuccessModal} onClose={() => setShowSuccessModal(false)} title="Transaksi Sukses!">
          <div className="text-center space-y-4">
              <div className="w-16 h-16 bg-green-100 text-green-600 rounded-full flex items-center justify-center mx-auto text-3xl"><i className="fa-solid fa-check"></i></div>
              ...
              <div className="grid grid-cols-2 gap-2">
                  <Button variant="outline" onClick={() => setShowSuccessModal(false)}>Selesai</Button>
      ...
      <Modal isOpen={showCustomerModal} onClose={() => setShowCustomerModal(false)} title="Pilih Member">
          ...
          <div className="mt-4 space-y-2 max-h-[300px] overflow-y-auto">
              {allCustomers.filter(c => c.name.toLowerCase().includes(customerSearch.toLowerCase())).map(c => (
                  <button key={c.id} onClick={() => { setSelectedCustomer(c); setShowCustomerModal(false); }} className="w-full flex justify-between p-3 border rounded-xl hover:bg-blue-50">
                      <span className="font-bold">{c.name}</span><Badge>{c.tier}</Badge>
                  </button>
              ))}
          </div>
      </Modal>

      <Modal isOpen={showScanner} onClose={() => setShowScanner(false)} title="Scan Barcode"><div id="reader" className="w-full max-w-[300px] min-h-[250px] mx-auto bg-black rounded-lg"></div></Modal>
    </div>
  );
};
export default POS;
