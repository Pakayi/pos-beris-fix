...
import { Product, ProductUnit, Supplier } from "../types";
  ...
  const handleImportExcel = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        if (confirm(`Impor ${rawData.length} data produk?`)) {
          setIsImporting(true);
          ...
          for (let i = 0; i < rawData.length; i++) {
            ...
            const newProduct: Product = {
              ...
              stock: stock,
              minStockAlert: 5,
              units: existing ? existing.units : [{ name: String(baseUnit), conversion: 1, price: price, buyPrice: buyPrice }],
              updatedAt: Date.now(),
            };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-200 pb-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            onClick={() => {
              setEditingProduct({ units: [{ name: "Pcs", conversion: 1, price: 0, buyPrice: 0 }], stock: "", minStockAlert: "", baseUnit: "Pcs", supplierId: "" });
              setIsModalOpen(true);
            }}
              ...
              {filtered.slice(0, 100).map((product) => {
                ...
                return (
                  <tr key={product.id} className={`hover:bg-gray-50 transition-colors ${isLowStock ? "bg-red-50" : ""}`}>
                    ...
                    <td className="px-4 py-3 text-slate-500 italic text-xs">{supplierName}</td>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex gap-1 flex-col">
              ...
              <Button onClick={() => setShowScanner(true)} size="sm" variant="secondary" icon="fa-solid fa-barcode">
                Scan
              </Button>
            </div>
            <div className="flex flex-col">
              <label className="block text-xs font-semibold text-gray-500 mb-1">Supplier Utama</label>
              <select
                className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <h4 className="text-xs font-bold text-gray-500 uppercase tracking-widest">Satuan & Harga</h4>
            ...
            {editingProduct.units?.map((unit: any, idx: number) => (
              <div key={idx} className={`p-3 rounded-xl border ${idx === 0 ? "bg-blue-50 border-blue-100" : "bg-gray-50 border-gray-200"} space-y-2 relative`}>
                <div className="grid grid-cols-2 gap-2">
                  <Input label={idx === 0 ? "Satuan Utama" : "Satuan"} value={unit.name} onChange={(e) => updateUnit(idx, "name", e.target.value)} />
                  <Input label="Konversi" type="number" disabled={idx === 0} value={unit.conversion} onChange={(e) => updateUnit(idx, "conversion", Number(e.target.value))} />
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <CurrencyInput label="HPP" value={unit.buyPrice} onChange={(val) => updateUnit(idx, "buyPrice", val)} />
