  ...
  useEffect(() => {
    if (showScanner) {
      ...
      const timer = setTimeout(async () => {
        ...
        } catch (err) { setCameraError("Kamera tidak dapat diakses."); }
      }, 300);
      return () => {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          {isOwner && (
            <>
              <Button onClick={handleExportCSV} variant="outline" icon="fa-file-csv">Export CSV</Button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex flex-col md:flex-row gap-4 items-center">
           ...
           <div className="text-xs text-gray-400">Total: <b>{filtered.length}</b> produk</div>
        </div>
                ...
                return (
                  <tr key={product.id} className={`hover:bg-gray-50 ${isLowStock ? 'bg-red-50' : ''}`}>
                    <td className="px-4 py-3">
                      ...
                      <div className="text-[10px] text-gray-400 font-mono">{product.category} | {product.sku || '-'}</div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-center gap-1">
                        <button 
                          onClick={() => { setStockAction({product, type: 'IN', quantity: '', unitIdx: 0, reason: ''}); setIsStockModalOpen(true); }}
                          className="p-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 border border-emerald-100" title="Kelola Stok"
                        >
                          <i className="fa-solid fa-boxes-stacked"></i>
                        </button>
                        {isOwner && (
                          <>
                            <button onClick={() => { setEditingProduct(product); setIsModalOpen(true); }} className="p-1.5 bg-blue-50 text-blue-600 rounded-lg border border-blue-100"><i className="fa-solid fa-pen"></i></button>
                            <button onClick={() => { if(confirm('Hapus produk ini secara permanen?')) { db.deleteProduct(product.id); refreshProducts(); }}} className="p-1.5 bg-red-50 text-red-600 rounded-lg border border-red-100"><i className="fa-solid fa-trash"></i></button>
                          </>
                        )}
                      </div>
                    </td>
          ...
          {stockAction.product && (
              <div className="space-y-4">
                  <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                      ...
                      <p className="text-xs text-blue-700 mt-1">Stok Saat Ini: <span className="font-bold">{stockAction.product.stock} {stockAction.product.baseUnit}</span></p>
                  </div>

                  <div className="flex bg-slate-100 p-1 rounded-lg">
                      <button onClick={() => setStockAction({...stockAction, type: 'IN'})} className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${stockAction.type === 'IN' ? 'bg-emerald-500 text-white shadow-md' : 'text-slate-500'}`}>BARANG MASUK (+)</button>
                      <button onClick={() => setStockAction({...stockAction, type: 'OUT'})} className={`flex-1 py-2 text-xs font-bold rounded-md transition-all ${stockAction.type === 'OUT' ? 'bg-red-500 text-white shadow-md' : 'text-slate-500'}`}>BARANG KELUAR (-)</button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                          ...
                          <select className="w-full p-2 border rounded-lg text-sm bg-white" value={stockAction.unitIdx} onChange={e => setStockAction({...stockAction, unitIdx: Number(e.target.value)})}>
                              {stockAction.product.units.map((u, i) => <option key={i} value={i}>{u.name}</option>)}
                          </select>
                      </div>
                  </div>

                  <Input label="Alasan / Catatan (Opsional)" value={stockAction.reason} onChange={e => setStockAction({...stockAction, reason: e.target.value})} placeholder={stockAction.type === 'IN' ? "Contoh: Belanja Supplier A" : "Contoh: Barang Rusak"} />

                  <div className="pt-4 flex gap-2">
                      <Button variant="secondary" className="flex-1" onClick={() => setIsStockModalOpen(false)}>Batal</Button>
                      <Button className="flex-1" onClick={handleUpdateStock}>Simpan Perubahan</Button>
                  </div>
              </div>
          )}
      </Modal>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingProduct.id ? "Edit Produk" : "Tambah Produk Baru"}>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div className="w-full">
                    ...
                    <div className="flex gap-1">
                        ...
                        <Button variant="secondary" onClick={() => setShowScanner(true)}><i className="fa-solid fa-barcode"></i></Button>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-3">
                <div className="flex justify-between items-center"><label className="text-xs font-bold text-gray-400 uppercase">Harga & Satuan Jual</label></div>
                {editingProduct.units?.map((unit: any, idx: number) => (
                    <div key={idx} className="p-4 border border-blue-100 bg-blue-50/20 rounded-xl space-y-3">
                        <div className="grid grid-cols-2 gap-3">
                            <Input label="Nama Satuan" value={unit.name} onChange={e => { const u = [...editingProduct.units]; u[idx].name = e.target.value; setEditingProduct({...editingProduct, units: u}); }} />
                            <Input label="Isi (Konversi)" type="number" value={unit.conversion} onChange={e => { const u = [...editingProduct.units]; u[idx].conversion = Number(e.target.value); setEditingProduct({...editingProduct, units: u}); }} />
                        </div>
                        <div className="grid grid-cols-2 gap-3">
                            {isOwner ? (
                              <CurrencyInput label="Modal (Beli)" value={unit.buyPrice || 0} onChange={val => { const u = [...editingProduct.units]; u[idx].buyPrice = val; setEditingProduct({...editingProduct, units: u}); }} />
                            ) : (
                            ...
                            <CurrencyInput label="Jual" value={unit.price} onChange={val => { const u = [...editingProduct.units]; u[idx].price = val; setEditingProduct({...editingProduct, units: u}); }} />
                        </div>
                    </div>
                ))}
            </div>
            <Button className="w-full py-3" onClick={handleSave}>Simpan Produk</Button>
          </div>
      </Modal>
