...
const Products: React.FC<ProductsProps> = ({ role }) => {
  ...
  const addUnitField = (e: React.MouseEvent) => {
    ...
    // Jika belum ada unit sama sekali, inisialisasi dengan base unit dulu
  ...
  const filtered = products.filter((p) => p.name.toLowerCase().includes(searchTerm.toLowerCase()) || (p.sku && p.sku.includes(searchTerm)));

  return (
    <div className="space-y-6">
      {/* Header Utama - Memperbaiki layout agar tidak bertumpuk */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2 w-full lg:w-auto">
          ...
          <Button variant="secondary" size="sm" onClick={exportToExcel} icon="fa-file-export">
            Export
          </Button>
          {isOwner && (
            <Button
              size="sm"
              onClick={() => {
                setEditingProduct({ units: [{ name: "Pcs", conversion: 1, price: 0, buyPrice: 0 }], baseUnit: "Pcs", stock: "", minStockAlert: "" });
                setIsModalOpen(true);
              }}
              icon="fa-plus"
            >
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="text-[10px] text-gray-400 font-bold bg-gray-50 px-3 py-1.5 rounded-full border shrink-0">{filtered.length} PRODUK DITEMUKAN</div>
        </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-center gap-2">
                        <button
                          ...
                          className="w-8 h-8 flex items-center justify-center bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 border border-emerald-100"
                          title="Update Stok"
                        >
                        ...
                        {isOwner && (
                          <>
                            <button
                              ...
                              className="w-8 h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-lg border border-blue-100"
                              title="Edit"
                            >
                            ...
                            <button
                              ...
                              className="w-8 h-8 flex items-center justify-center bg-red-50 text-red-600 rounded-lg border border-red-100"
                              title="Hapus"
                            >
          ...
          <div className="space-y-4">
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl">
              <button
                ...
                className={`flex-1 py-2 text-[10px] font-bold rounded-lg transition-all ${stockAction.type === "IN" ? "bg-emerald-500 text-white shadow" : "text-slate-500"}`}
              >
                RESTOCK (+)
              </button>
              <button
                onClick={() => setStockAction({ ...stockAction, type: "OUT" })}
                className={`flex-1 py-2 text-[10px] font-bold rounded-lg transition-all ${stockAction.type === "OUT" ? "bg-red-500 text-white shadow" : "text-slate-500"}`}
              >
                PENYESUAIAN (-)
              </button>
            </div>
            ...
            <Button className="w-full py-3 font-bold" onClick={handleUpdateStock}>
              Simpan Perubahan
            </Button>
          </div>
        )}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <Input value={editingProduct.sku || ""} onChange={(e) => setEditingProduct({ ...editingProduct, sku: e.target.value })} placeholder="Scan atau ketik" className="flex-1" />
