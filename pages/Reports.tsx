...
const Reports: React.FC = () => {
  ...
  const runAiAnalysis = async () => {
    ...
    try {
      ...
      setAiAnalysis("Kunci API Gemini tidak valid atau sedang sibuk.");
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex bg-white rounded-lg p-1 border">
          {["today", "week", "month"].map((r: any) => (
            <button key={r} onClick={() => setRange(r)} className={`px-4 py-1.5 text-xs font-bold rounded-md ${range === r ? "bg-blue-600 text-white" : "text-gray-500"}`}>
              {r}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="p-6 border-l-4 border-blue-500">
          <p className="text-xs text-gray-400 font-bold uppercase">Omzet Periode Ini</p>
          <p className="text-2xl font-black">Rp {stats.revenue.toLocaleString("id-ID")}</p>
        </Card>
        <Card className="p-6 bg-blue-700 text-white flex justify-between items-center">
          <div>
            <p className="font-bold">Analisis AI</p>
            <p className="text-xs opacity-70">Dapatkan saran bisnis</p>
          </div>
          <Button onClick={runAiAnalysis} variant="secondary" size="sm">
            Tanya AI
          </Button>
        </Card>
      </div>
      <Card className="p-6">
        <h3 className="font-bold mb-4">Top Produk</h3>
        <div className="space-y-2">
          {stats.top.map((p, i) => (
            <div key={i} className="flex justify-between p-3 bg-gray-50 rounded-lg">
              <span className="font-bold">{p[0]}</span>
              <Badge>{p[1]} terjual</Badge>
            </div>
          ))}
        </div>
      </Card>
