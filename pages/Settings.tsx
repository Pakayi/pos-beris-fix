...
const Settings: React.FC = () => {
  ...
  const handleConnectPrinter = async () => {
    ...
    } catch (error: any) { alert(error.message); } finally { setIsConnectingPrinter(false); }
  };
  ...
  const handlePinAction = (inputPin: string) => {
    if (pinMode === 'create') { setTempPin(inputPin); setPinMode('confirm_create'); }
      ...
      } else { setPinError("PIN tidak cocok."); setPinMode('create'); }
  ...
  // Get current warung ID safely from profile
  const getWarungId = () => {
    try {
      const profile = JSON.parse(localStorage.getItem('warung_user_profile') || '{}');
      return profile.warungId || 'N/A';
    } catch {
      return 'N/A';
    }
  };
        ...
        {[
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 text-sm font-bold rounded-lg transition-all whitespace-nowrap ${
              activeTab === tab.id ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'text-slate-500 hover:bg-slate-50'
            }`}
          >
            ...
            <Card className="p-6 space-y-4">
              ...
              <div className="flex gap-4 mb-4">
                ...
                <div className="flex-1">
                   ...
                   {settings.logoUrl && <button onClick={() => setSettings({...settings, logoUrl: null})} className="text-xs text-red-500">Hapus Logo</button>}
                </div>
              </div>
            ...
            <Card className="p-6 space-y-4">
              ...
              <div className="p-4 bg-indigo-50 rounded-xl border border-indigo-100">
                ...
                {printerStatus === 'connected' ? (
                   <Button size="sm" variant="danger" onClick={handleDisconnectPrinter}>Putus Koneksi</Button>
                ) : (
                   <Button size="sm" onClick={handleConnectPrinter} disabled={isConnectingPrinter}>{isConnectingPrinter ? 'Searching...' : 'Hubungkan Printer'}</Button>
                )}
              </div>
          ...
          <Card className="p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="bg-blue-50 text-blue-700 px-4 py-2 rounded-lg border border-blue-100 font-mono text-sm">
                ID Warung: <span className="font-bold select-all cursor-pointer">{getWarungId()}</span>
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-gray-100">
                  {loadingStaff ? (
                    <tr><td colSpan={5} className="py-10 text-center text-gray-400">Memuat data staf...</td></tr>
                  ) : staff.map(user => (
                      ...
                      <td className="px-4 py-3 text-right">
                        {user.role !== 'owner' && (
                          <Button 
                            ...
                            size="sm"
                            onClick={() => handleToggleStaffStatus(user.uid, user.active)}
                          >
              ...
              <Card className="p-6 space-y-4">
                 ...
                 <div className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border">
                    ...
                    <Button 
                      ...
                      size="sm"
                      onClick={() => { setPinMode(settings.securityPin ? 'verify_disable' : 'create'); setShowPinModal(true); }}
                    >
      ...
      {showPinModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
           <PinPad 
              ...
              title={pinMode === 'create' ? "Buat PIN Baru" : pinMode === 'confirm_create' ? "Ulangi PIN" : "Verifikasi PIN"}
              error={pinError}
           />
        </div>
      )}
