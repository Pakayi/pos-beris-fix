    ...
    <div className="relative">
      {prefix && (
        <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm flex items-center justify-center w-5">
          {prefix.startsWith("fa-") ? <i className={`fa-solid ${prefix}`}></i> : <span className="font-medium">{prefix}</span>}
        </div>
      )}
      <input
        ref={ref}
        className={`w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all ${prefix ? "pl-10" : ""} ${
